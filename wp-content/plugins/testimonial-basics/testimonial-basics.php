 ...
 */
if ( isset( $_SERVER['SCRIPT_FILENAME'] ) && 'testimonial-basics.php' === basename( sanitize_text_field( wp_unslash( $_SERVER['SCRIPT_FILENAME'] ) ) ) ) { // Input var okay.
	die( esc_html__( 'Please do not access this file directly. Thanks!', 'testimonial-basics' ) );
}
...
function katb_testimomial_basics_activate() {
	...
	if ( $wpdb->get_var( "SHOW TABLES LIKE '$tablename'" ) !== $tablename && $wpdb->get_var( "SHOW TABLES LIKE '$tablename'" ) !== $tableprefix . 'testimonial_basics' ) { // WPCS: db call ok, cache ok.
...
function katb_load_scripts() {
	...
	if ( true === $katb_options['katb_widget_use_excerpts'] || true === $katb_options['katb_use_excerpts'] ) {
		wp_enqueue_script( 'katb_excerpt_js', plugins_url() . '/testimonial-basics/js/katb_excerpt_doc_ready.js', array( 'jquery' ), '', true );
	}
	if ( true === $katb_options['katb_enable_rotator'] ) {
		wp_enqueue_script( 'katb_rotator_js', plugins_url() . '/testimonial-basics/js/katb_rotator_doc_ready.js', array( 'jquery' ), '', true );
		wp_enqueue_script( 'jquery-effects-slide' );
	}
	if ( true === $katb_options['katb_use_recaptcha'] ) {
		// CaptchaCallback is in the doc ready so it is loaded first.
		wp_enqueue_script( 'katb-recaptcha-doc-ready', plugins_url() . '/testimonial-basics/js/katb_recaptcha.js', array( 'jquery' ), '', true );
		wp_enqueue_script( 'katb-google-recaptcha', 'https://www.google.com/recaptcha/api.js?onload=KatbCaptchaCallback&render=explicit', array( 'katb-recaptcha-doc-ready' ), '', true );
	}
	wp_enqueue_script( 'katb_mosaic_js', plugins_url() . '/testimonial-basics/js/katb_mosaic_doc_ready.js', array( 'jquery' ), '1.0.0', true );
}
