...
function katb_list_testimonials( $atts ) {
	...
	ob_start();
	if ( '' !== $katb_error ) {
		echo '<div class="katb_error">' . $katb_error . '</div>'; // WPCS: XSS ok.
	} else {
	...
	if ( true === $display_reviews && true === $use_schema ) {
		if ( isset( $katb_options['katb_use_pagination'] ) && true === $katb_options['katb_use_pagination'] && isset( $katb_paginate_setup ) ) {
			echo katb_get_display_pagination_string( $katb_paginate_setup, $use_formatted_display ); // WPCS: XSS ok.
		}
	} else {
		if ( isset( $katb_options['katb_use_pagination'] ) && true === $katb_options['katb_use_pagination'] && isset( $katb_paginate_setup ) ) {
			echo katb_get_display_pagination_string( $katb_paginate_setup, $use_formatted_display ); // WPCS: XSS ok.
	...
	if ( isset( $_POST[ 'katb_submitted' . $katb_input_form_no ], $_POST[ 'katb_main_form_nonce' . $katb_input_form_no ] ) && // Input var okay.
		...
		if ( false === $katb_bot_submission ) {
			...
			if ( '' === $katb_input_error ) {
				...
				if ( true === $use_popup ) {
					$error_message = esc_html__( 'There were errors so the testimonial was not added: ', 'testimonial-basics' ) . $katb_input_error; // WPCS: XSS ok.
					?>
					<script>alert("<?php echo $error_message; // WPCS: XSS ok. ?>")</script>
					<?php
				} else {
					$input_html .= '<span class="katb_error">' . esc_html__( 'There were errors so the testimonial was not added: ', 'testimonial-basics' ) . $katb_input_error . '</span>'; // WPCS: XSS ok.
				}
			}
		}
	}
		...
		if ( false === $katb_bot_submission ) {
			...
			if ( '' === $katb_remove_error ) {
				...
				if ( true === $use_popup ) {
					...
					$html .= '<span class="katb_error">' . esc_html__( 'There were errors so the request was not sent: ', 'testimonial-basics' ) . $katb_remove_error . '</span>'; // WPCS: XSS ok.
				}
			}
		}
	}
