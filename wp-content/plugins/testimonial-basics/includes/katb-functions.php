	...
	if ( true === $use_ratings ) {
		...
		if ( 0 < $rating ) {
			...
			<span class="katb_css_rating">
				<?php echo katb_css_rating( $rating ); // WPCS: XSS ok. ?>
			</span>
			<?php
			if ( true === $use_schema ) {
				?>
...
function katb_get_content_html( $use_schema, $format, $use_excerpts, $length, $gravatar_or_photo, $content, $id, $source ) {
	...
	if ( true === $use_excerpts ) {
		...
		?>
		<div class="katb_test_text<?php echo esc_attr( $format ); ?>" <?php echo $schema_markup;// WPCS: XSS ok. ?>>
		<?php
			echo $gravatar_or_photo; // WPCS: XSS ok.
...
function katb_get_gdpr_link( $gdpr_remove_permalink, $tb_id, $divider, $widget_or_content ) {
	...
	if ( true === $use_gdpr && '' !== trim( $gdpr_remove_permalink ) ) {
		?>
		<span class="katb_remove_link <?php echo $widget_or_content; // WPCS: XSS ok. ?>">
...
function katb_schema_aggregate_markup( $display_aggregate, $group_name, $use_group_name_for_aggregate, $custom_aggregate_name, $layout ) {
	...
	// Query database.
	if ( 'all' !== $group_name ) {
		$aggregate_data           = $wpdb->get_results( " SELECT `tb_rating` FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_group` = '$group_name' ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		$aggregate_total_approved = $wpdb->num_rows;
	} else {
		$aggregate_data           = $wpdb->get_results( " SELECT `tb_rating` FROM `$tablename` WHERE `tb_approved` = '1' ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		$aggregate_total_approved = $wpdb->num_rows;
	}
	$count = 0;
	$sum   = 0;
...
function katb_get_testimonials_from_ids( $id ) {
	...
	foreach ( $id_picks_processed as $pick ) {
		...
		$tdata   = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_id` = '$pick_id' ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
...
function katb_get_testimonials( $group, $number, $by, $rotate, $use_pagination ) {
	...
	if ( 'all' === $group && 'all' === $number && 'date' === $by ) {
		if ( isset( $use_pagination ) && true === $use_pagination && 'no' === $rotate ) {
			...
			// Get total entries.
			$results       = $wpdb->get_results( " SELECT COUNT(1) FROM `$tablename` WHERE `tb_approved` = '1' ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
			...
			if ( 0 > $katb_offset ) {
				$katb_offset = 0; }
			// Get results.
			$katb_tdata = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' ORDER BY `tb_date` DESC LIMIT $katb_items_per_page OFFSET $katb_offset ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		} else {
			$katb_tdata = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' ORDER BY `tb_date` DESC ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		}
		...
		if ( isset( $use_pagination ) && true === $use_pagination && 'no' === $rotate ) {
			...
			if ( 0 > $katb_offset ) {
				$katb_offset = 0; }
			// Get results.
			$katb_tdata = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' ORDER BY `tb_order` = '0', `tb_order` ASC,`tb_date` DESC LIMIT $katb_items_per_page OFFSET $katb_offset ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		} else {
			$katb_tdata = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' ORDER BY `tb_order` = '0', `tb_order` ASC,`tb_date` DESC ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		}
		$katb_tnumber = $wpdb->num_rows;
	} elseif ( 'all' === $group && 'all' === $number && 'random' === $by ) {
		$katb_tdata   = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' ORDER BY RAND() ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		$katb_tnumber = $wpdb->num_rows;
	} elseif ( 'all' === $group && 'all' !== $number && 'date' === $by ) {
		$katb_tdata   = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' ORDER BY `tb_date` DESC LIMIT 0,$number ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		$katb_tnumber = $wpdb->num_rows;
	} elseif ( 'all' === $group && 'all' !== $number && 'order' === $by ) {
		$katb_tdata   = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' ORDER BY `tb_order` = '0',`tb_order` ASC LIMIT 0,$number ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		$katb_tnumber = $wpdb->num_rows;
	} elseif ( 'all' === $group && 'all' !== $number && 'random' === $by ) {
		$katb_tdata   = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' ORDER BY RAND() LIMIT 0,$number ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		$katb_tnumber = $wpdb->num_rows;
	} elseif ( 'all' !== $group && 'all' === $number && 'date' === $by ) {
		if ( isset( $use_pagination ) && true === $use_pagination && 'no' === $rotate ) {
			// Setup Pagination.
			...
			$katb_items_per_page = intval( $katb_options['katb_paginate_number'] );
			// Get total entries.
			$results       = $wpdb->get_results( " SELECT COUNT(1) FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_group` = '$group' ", ARRAY_A );// WPCS: db call ok,unprepared SQL ok, cache ok.
			$total_entries = $results[0]['COUNT(1)'];
			// Check for offset.
			if ( isset( $_POST['ka_paginate_post'], $_POST['katb_paginate_form_nonce'] ) && // Input var okay.
			wp_verify_nonce( sanitize_key( $_POST['katb_paginate_form_nonce'] ), 'katb_paginate_nonce' ) ) { // WPCS: input var ok.
			...
			if ( $katb_offset < 0 ) {
				$katb_offset = 0; }
			// Get results.
			$katb_tdata = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_group` = '$group' ORDER BY `tb_date` DESC LIMIT $katb_items_per_page OFFSET $katb_offset ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		} else {
			$katb_tdata = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_group` = '$group' ORDER BY `tb_date` DESC ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		}
		$katb_tnumber = $wpdb->num_rows;
	} elseif ( 'all' !== $group && 'all' === $number && 'order' === $by ) {
		if ( isset( $use_pagination ) && true === $use_pagination && 'no' === $rotate ) {
			...
			if ( 0 > $katb_offset ) {
				$katb_offset = 0; }
			// Get results.
			$katb_tdata = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_group` = '$group' ORDER BY `tb_order` = '0',`tb_order` ASC,`tb_date` DESC LIMIT $katb_items_per_page OFFSET $katb_offset ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		} else {
			$katb_tdata = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_group` = '$group' ORDER BY `tb_order` = '0',`tb_order` ASC,`tb_date` DESC ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		}
		$katb_tnumber = $wpdb->num_rows;
	} elseif ( 'all' !== $group && 'all' === $number && 'random' === $by ) {
		$katb_tdata   = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_group` = '$group' ORDER BY RAND() ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		$katb_tnumber = $wpdb->num_rows;
	} elseif ( 'all' !== $group && 'all' !== $number && 'date' === $by ) {
		$katb_tdata   = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_group` = '$group' ORDER BY `tb_date` DESC LIMIT 0,$number ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		$katb_tnumber = $wpdb->num_rows;
	} elseif ( 'all' !== $group && 'all' !== $number && 'order' === $by ) {
		$katb_tdata   = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_group` = '$group' ORDER BY `tb_order` = '0',`tb_order` ASC LIMIT 0,$number ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		$katb_tnumber = $wpdb->num_rows;
	} elseif ( 'all' !== $group && 'all' !== $number && 'random' === $by ) {
		$katb_tdata   = $wpdb->get_results( " SELECT * FROM `$tablename` WHERE `tb_approved` = '1' AND `tb_group` = '$group' ORDER BY RAND() LIMIT 0,$number ", ARRAY_A ); // WPCS: db call ok,unprepared SQL ok, cache ok.
		$katb_tnumber = $wpdb->num_rows;
	}
	$katb_tdata_array[0] = $katb_tdata;
...
function katb_testimonial_basics_display_in_code( $group = 'all', $number = 'all', $by = 'random', $id = '', $rotate = 'no', $layout = '0', $schema = 'default' ) {
	...
	echo katb_list_testimonials( $atts ); // WPCS: XSS ok.
}
...
function katb_testimonial_basics_input_in_code( $group = 'All', $form = '1' ) {
	...
	echo katb_display_input_form( $atts );  // WPCS: XSS ok.
}
