...
function katb_testimonial_basic_admin_style( $hook ) {
	...
	if ( is_rtl() && false === $katb_options['katb_remove_rtl_support'] ) {
		wp_enqueue_style( 'katb_admin_style', plugins_url() . '/testimonial-basics/css/katb_testimonial_basics_admin_rtl.css', array(), '', 'all' );
	} else {
		wp_enqueue_style( 'katb_admin_style', plugins_url() . '/testimonial-basics/css/katb_testimonial_basics_admin.css', array(), '', 'all' );
	}
	...
	wp_enqueue_media();
	wp_enqueue_script( 'katb_options_js', plugins_url() . '/testimonial-basics/js/katb_testimonial_basics_doc_ready.js', array( 'jquery' ), '', true );
}
...
function katb_testimonial_basics_introduction(){ ?>
	...
	<?php echo katb_intro_html(); // WPCS: XSS ok. ?>
	<h3>
...
function katb_testimonial_basics_edit_page() {
	...
	// submit testimonial.
	if ( isset( $_POST['submitted'] ) && check_admin_referer( 'katb_nonce_3', 'katb_admin_form_nonce' ) ) { // WPCS: input var ok.
		// Validate Input.
		$error = '';
		if ( isset( $_POST['tb_id'] ) && '' !== $_POST['tb_id'] ) {
			$katb_id = intval( wp_unslash( $_POST['tb_id'] ) ); // WPCS: input var ok.
		} else {
			$katb_id = '';
		}
		// Order must be an integer.
		if ( isset( $_POST['tb_order'] ) && '' !== $_POST['tb_order'] ) {
			$katb_order = intval( wp_unslash( $_POST['tb_order'] ) ); // WPCS: input var ok.
		} else {
			$katb_order = 0;
		}
		// Approved is either checked (1) or not checked (0).
		if ( ! isset( $_POST['tb_approved'] ) ) { // WPCS: input var ok.
			$katb_approved = 0;
		} else {
			$katb_approved = 1;
		}
		...
		if ( ! empty( $_POST['tb_group'] ) ) {
			$katb_group = sanitize_text_field( wp_unslash( $_POST['tb_group'] ) ); // WPCS: input var ok.
		} else {
			$katb_group = '';
		}
		// author validation.
		if ( ! empty( $_POST['tb_author'] ) ) {
			$katb_author = sanitize_text_field( wp_unslash( $_POST['tb_author'] ) ); // WPCS: input var ok.
		} else {
			$katb_author = '';
			$error      .= '*' . esc_html__( 'Author is required', 'testimonial-basics' ) . '*';
		}
		// website validation.
		if ( ! empty( $_POST['tb_website'] ) ) { // WPCS: input var ok.
			$katb_website = esc_url_raw( wp_unslash( $_POST['tb_website'] ) ); // WPCS: input var ok.
		...
		if ( ! empty( $_POST['tb_location'] ) ) {
			$katb_location = sanitize_text_field( wp_unslash( $_POST['tb_location'] ) ); // WPCS: input var ok.
		} else {
			$katb_location = '';
		}
		// custom1 validation.
		if ( ! empty( $_POST['tb_custom1'] ) ) {
			$katb_custom1 = sanitize_text_field( wp_unslash( $_POST['tb_custom1'] ) ); // WPCS: input var ok.
		} else {
			$katb_custom1 = '';
		}
		// custom2 validation.
		if ( ! empty( $_POST['tb_custom2'] ) ) {
			$katb_custom2 = sanitize_text_field( wp_unslash( $_POST['tb_custom2'] ) ); // WPCS: input var ok.
		} else {
			$katb_custom2 = '';
		}
		// email validation.
		if ( ! empty( $_POST['tb_email'] ) ) {
			$katb_email = sanitize_email( wp_unslash( $_POST['tb_email'] ) ); // WPCS: input var ok.
		} else {
			$katb_email = '';
		}
		// Date Validation.
		if ( ! empty( $_POST['tb_date'] ) ) {
			$katb_date = sanitize_text_field( wp_unslash( $_POST['tb_date'] ) ); // WPCS: input var ok.
		} else {
			$katb_date = '';
		}
		if ( '' !== $katb_date ) {
		...
		if ( ! empty( $_POST['tb_time'] ) ) {
			$katb_time = sanitize_text_field( wp_unslash( $_POST['tb_time'] ) ); // WPCS: input var ok.
		} else {
			$katb_time = '';
		}
		if ( '' !== $katb_time ) {
		...
		if ( ! empty( $_POST['tb_rating'] ) ) {
			$katb_rating = sanitize_text_field( wp_unslash( $_POST['tb_rating'] ) ); // WPCS: input var ok.
		} else {
			$katb_rating = '';
		}
		// photo validation.
		if ( ! empty( $_POST['tb_upload_image'] ) ) { // WPCS: input var ok.
			$katb_picture_url = esc_url_raw( wp_unslash( $_POST['tb_upload_image'] ) ); // WPCS: input var ok.
		...
		if ( ! empty( $_POST['tb_title'] ) ) {
			$katb_title = sanitize_text_field( wp_unslash( $_POST['tb_title'] ) ); // WPCS: input var ok.
		} else {
			$katb_title = '';
		}
		...
		if ( ! empty( $_POST['tb_testimonial'] ) ) {
			$katb_fix_emoticons = wp_encode_emoji( wp_unslash( $_POST['tb_testimonial'] ) ); // WPCS: input var ok, sanitization ok.
		...
		if ( '' === $error ) {
			...
			if ( '' === $katb_id || '0' === $katb_id ) {
				...
				$prev_entry = $wpdb->get_row( "SELECT * FROM `$tablename` ORDER BY `tb_id` DESC ", ARRAY_A ); // WPCS: db call ok, cache ok, unprepared SQL ok.
				if ( $prev_entry['tb_testimonial'] !== $katb_testimonial ) {
					$wpdb->insert( $tablename, $values, $formats_values ); // WPCS: db call ok, cache ok, unprepared SQL ok.
				...
				$wpdb->update( $tablename, $values, $where, $formats_values );  // WPCS: db call ok, cache ok, unprepared SQL ok.
				echo '<div id="message" class="updated">' . esc_html__( 'Testimonial updated.', 'testimonial-basics' ) . '</div>';
			}
		} else {
			echo '<div id="message" class="error">' . esc_html__( 'Error,testimonial was not added', 'testimonial-basics' ) . ': ' . $error . '</div>'; // WPCS: XSS ok.
		}
	} else {
		$katb_id          = '';
		$katb_order       = 0;
	...
	if ( isset( $_POST['reset'] ) && check_admin_referer( 'katb_nonce_3', 'katb_admin_form_nonce' ) ) { // WPCS: input var ok.
		$katb_id          = '';
		$katb_order       = 0;
		$katb_approved    = 0;
		$katb_group       = '';
	...
	if ( isset( $_POST['delete'] ) && check_admin_referer( 'katb_nonce_3', 'katb_admin_form_nonce' ) ) { // WPCS: input var ok.
		if ( ! empty( $_POST['tb_id'] ) ) {
			$katb_id = sanitize_text_field( wp_unslash( $_POST['tb_id'] ) ); // WPCS: input var ok.
		} else {
			$katb_id = '';
		}
		if ( '' === $katb_id ) {
			...
			$wpdb->query( " DELETE FROM `$tablename` WHERE `tb_id`=$katb_id " ); // WPCS: db call ok, cache ok, unprepared SQL ok, xss ok.
			$katb_id          = '';
			$katb_order       = '';
			$katb_approved    = '';
			$katb_group       = '';
	...
	if ( isset( $_POST['edit'] ) && check_admin_referer( 'katb_nonce_4', 'katb_admin_form_nonce_2' ) ) { // WPCS: input var ok.
		$katb_id          = sanitize_text_field( wp_unslash( $_POST['edit'] ) ); // WPCS: input var ok.
		$edit_data        = $wpdb->get_row( "SELECT * FROM `$tablename` WHERE `tb_id` = $katb_id ", ARRAY_A ); // WPCS: db call ok, cache ok, unprepared SQL ok.
	...
	/* ------- set up pagination ----------- */
	if ( isset( $_POST['ka_paginate_post'], $_POST['katb_paginate_form_nonce'] ) && // Input var okay.
		wp_verify_nonce( sanitize_key( $_POST['katb_paginate_form_nonce'] ), 'katb_paginate_nonce' ) ) { // WPCS: input var ok.
		// Get total entries.
		$results            = $wpdb->get_results( " SELECT COUNT(1) FROM `$tablename` ", ARRAY_A ); // WPCS: db call ok, cache ok, unprepared SQL ok.
		$total_entries      = $results[0]['COUNT(1)'];
		$ka_paginate_action = sanitize_text_field( wp_unslash( $_POST['ka_paginate_post'] ) ); // WPCS: input var ok.
...
?>
	<div class="wrap katb_admin_edit_wrap">
		...
		<?php katb_intro_html(); // WPCS: XSS ok. ?>
		...
		<h3 class="katb_admin_title"><?php esc_html_e( 'Testimonials', 'testimonial-basics' ); ?></h3>
			...
			$katb_tdata   = $wpdb->get_results( " SELECT * FROM `$tablename` ORDER BY `tb_date` DESC LIMIT $katb_items_per_page OFFSET $katb_admin_offset ", ARRAY_A ); // WPCS: db call ok, cache ok, unprepared SQL ok.
			$katb_tnumber = $wpdb->num_rows;
			// Bulk Delete.
			if ( isset( $_POST['bulk_delete'] ) && check_admin_referer( 'katb_nonce_4', 'katb_admin_form_nonce_2' ) ) { // Input var okay.
				for ( $i = 0; $i < $katb_tnumber; $i++ ) {
					if ( isset( $_POST[ 'bulk_delete-' . $katb_tdata[ $i ]['tb_id'] ] ) && '1' === $_POST[ 'bulk_delete-' . $katb_tdata[ $i ]['tb_id'] ] ) { // Input var okay.
						if ( '' === $katb_tdata[ $i ]['tb_id'] ) {
							...
							$wpdb->query( " DELETE FROM `$tablename` WHERE `tb_id`=$delete_id " ); // WPCS: db call ok, cache ok, unprepared SQL ok, xss ok.
						}
					}
				}
...
function katb_admin_msgs() {
	...
	if ( isset( $_GET['page'] ) ) { // WPCS: input var ok.
		$katb_settings_pg = strpos( $_GET['page'], 'katb-settings' ); // WPCS: input var ok, sanitization ok.
	} else {
	...
	if ( current_user_can( 'manage_options' ) && false !== $katb_settings_pg && ! empty( $set_errors ) ) {
		...
		if ( 'settings_updated' === $set_errors[0]['code'] && isset( $_GET['settings-updated'] ) ) { // WPCS: input var ok.
...
?>
	<div class="katb_paypal"><?php esc_html_e( 'Show your appreciation!', 'testimonial-basics' ); ?>
		<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank"> <?php //url ok. ?>
			<input type="hidden" name="cmd" value="_s-xclick">
			<input type="hidden" name="hosted_button_id" value="PP4GPMXBUVPY4">
			<input type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_donateCC_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!"> <?php //url ok. ?>
			<img alt="" border="0" src="https://www.paypalobjects.com/en_US/i/scr/pixel.gif" width="1" height="1"> <?php //url ok. ?>
		</form>
	</div>
	<p>
		<?php
		global $katb_options;
		if ( is_rtl() && false === $katb_options['katb_remove_rtl_support'] ) {
			echo '<a href="//www.kevinsspace.ca/testimonial-basics-wordpress-plugin/" target="_blank" >www.kevinsspace.ca/testimonial-basics-wordpress-plugin/</a>' . esc_html__( ' : Plugin Site', 'testimonial-basics' ); //url ok.
			echo '&nbsp;&nbsp;&nbsp;<a href="//www.kevinsspace.ca" target="_blank" >www.kevinsspace.ca</a>' . esc_html__( ' : Author Site', 'testimonial-basics' ); //url ok.
		} else {
			echo esc_html__( 'Author Site : ', 'testimonial-basics' ) . '<a href="//www.kevinsspace.ca" target="_blank" >www.kevinsspace.ca</a>&nbsp;&nbsp;&nbsp;'; //url ok.
			echo esc_html__( 'Plugin Site : ', 'testimonial-basics' ) . '<a href="//www.kevinsspace.ca/testimonial-basics-wordpress-plugin/" target="_blank" >www.kevinsspace.ca/testimonial-basics-wordpress-plugin/</a>'; //url ok.
		}
		?>
	</p>
<?php
...
function katb_get_current_tab() {
	$page = 'katb-settings';
	if ( isset( $_GET['tab'] ) ) {
		$current = sanitize_text_field( wp_unslash( $_GET['tab'] ) ); // WPCS: input var ok.
	} else {
		$current = 'general';
	}
	return $current;
...
function katb_maybe_download() {
	...
	if ( empty( $_POST['action'] ) || 'katb-backup-action' !== $_POST['action'] ) { // WPCS: input var ok.
		return;
	}
...
function katb_maybe_upload() {
	...
	if ( empty( $_POST['action'] ) || 'katb-import-action' !== $_POST['action'] ) { // WPCS: input var ok.
		return;
	}
	// Check permissions and nonces.
	if ( ! current_user_can( 'manage_options' ) ) {
	...
	// Sanity check.
	if ( empty( $_FILES['katb_import_file'] ) ) { // WPCS: input var ok.
		wp_die( 'No file found' );
	}
	$file = $_FILES['katb_import_file']; // WPCS: input var ok, sanitization ok.
...
function katb_do_backup( $args = array() ) {
	...
	$katb_tdata   = $wpdb->get_results( " SELECT * FROM `$table` ORDER BY `tb_date` DESC ", ARRAY_A ); // WPCS: db call ok, cache ok, unprepared SQL ok.
	$katb_tnumber = $wpdb->num_rows;
	if ( 0 === $katb_tnumber ) {
		return false;
	}
	...
	<testimonials>
		<?php for ( $i = 0; $i < $katb_tnumber; $i++ ) { ?>
			<testimonial>
				<id><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_id'] ) ); // WPCS XSS ok. ?></id>
				<date><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_date'] ) ); // WPCS XSS ok. ?></date>
				<group><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_group'] ) ); // WPCS XSS ok. ?></group>
				<order><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_order'] ) ); // WPCS XSS ok. ?></order>
				<approved><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_approved'] ) ); // WPCS XSS ok. ?></approved>
				<name><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_name'] ) ); // WPCS XSS ok. ?></name>
				<location><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_location'] ) ); // WPCS XSS ok. ?></location>
				<email><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_email'] ) ); // WPCS XSS ok. // WPCS XSS ok. ?></email>
				<pic_url><?php echo katb_wrap_cdata( esc_url( $katb_tdata[ $i ]['tb_pic_url'] ) ); // WPCS XSS ok. ?></pic_url>
				<web_url><?php echo katb_wrap_cdata( esc_url( $katb_tdata[ $i ]['tb_url'] ) ); // WPCS XSS ok. ?></web_url>
				<rating><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_rating'] ) ); // WPCS XSS ok. ?></rating>
				<custom1><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_custom1'] ) ); // WPCS XSS ok. ?></custom1>
				<custom2><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_custom2'] ) ); // WPCS XSS ok. ?></custom2>
				<title><?php echo katb_wrap_cdata( sanitize_text_field( $katb_tdata[ $i ]['tb_title'] ) ); // WPCS XSS ok. ?></title>
				<content><?php echo katb_wrap_cdata( wp_kses_post( $katb_tdata[ $i ]['tb_testimonial'] ) ); // WPCS XSS ok. ?></content>
			</testimonial>
		<?php } ?>
	</testimonials>
	<?php
...
function katb_import( $file ) {
	...
	global $wpdb;
	$table        = $wpdb->prefix . 'testimonial_basics';
	$katb_tdata   = $wpdb->get_results( " SELECT `tb_id` FROM `$table` ", ARRAY_A ); // WPCS: db call ok, cache ok, unprepared SQL ok.
	$katb_tnumber = $wpdb->num_rows;
	...
	foreach ( $testimonials as $testimonial ) {
		...
		$wpdb->insert( $table, $values, $formats_values ); // WPCS: db call ok, cache ok, unprepared SQL ok.
		$imported++;
	}
	return $imported;
}
...
function katb_admin_notices() {
	...
	if ( ! isset( $_GET['imported'] ) || 'testimonials_page_katb-backup' !== get_current_screen()->id ) { // WPCS: input var ok.
		return;
	}
	$imported = intval( wp_unslash( $_GET['imported'] ) ); // WPCS: input var ok.
	if ( true === $imported ) {
		printf( '<div class="updated"><p>%s</p></div>', esc_html__( '1 testimonial successfully imported', 'testimonial-basics' ) );
	} elseif ( intval( wp_unslash( $_GET['imported'] ) ) ) { // WPCS: input var ok.
...
function katb_add_unapproved_count() {
	...
	$tablename = $wpdb->prefix . 'testimonial_basics';
	$results   = $wpdb->get_results( " SELECT COUNT(1) FROM `$tablename` WHERE `tb_approved` = '0' ", ARRAY_A ); // WPCS: db call ok, cache ok, unprepared SQL ok.
	$total     = $results[0]['COUNT(1)'];
	if ( 0 !== $total ) {
		foreach ( $menu as $key => $value ) {
			if ( 'katb_testimonial_basics_admin' === $menu[ $key ][2] ) {
				$menu[ $key ][0] .= " <span class='update-plugins count-$total'><span class='plugin-count'>" . $total . '</span></span>'; // WPCS: override ok.
				return;
			}
		}
	}
