			...
			if ( false === $katb_bot_submission ) {
				...
				if ( '' === $katb_widget_html_error && '' === $katb_widget_popup_error ) {
					...
					if ( true === $use_widget_popup ) {
						...
						?>
						<script>alert("<?php echo $widget_error_message; // WPCS: XSS ok. ?>")</script>
						<?php
					} else {
							echo '<div class="katb_widget_error">' . esc_html__( 'There were errors so the testimonial was not added: ', 'testimonial-basics' ) . $katb_widget_html_error . '</div>'; // WPCS: XSS ok.
					}
				}
			}
		}
				...
				if ( true === $katb_options['katb_use_captcha'] ) {
					...
					<div class="katb_widget_captcha">
						...
						if ( true === $katb_options['katb_use_recaptcha'] ) {
							...
							echo katb_color_captcha_2( 'widget', $katb_widget_input_form_no ); // WPCS: XSS ok.
							?>
							...
							echo katb_color_captcha( 'widget', $katb_widget_input_form_no ); // WPCS: XSS ok.
							?>
							<input class="katb_captcha_widget_input"
									type="text"
									id="verify_widget_<?php echo esc_attr( $katb_widget_input_form_no ); ?>"
							...
							echo katb_bw_captcha( 'widget', $katb_widget_input_form_no ); // WPCS: XSS ok.
							?>
							<input class="katb_captcha_widget_input"
									type="text"
									id="verify_widget_<?php echo esc_attr( $katb_widget_input_form_no ); ?>"
			...
			if ( true === $katb_options['katb_show_widget_gravatar_link'] ) {
				...
				<span class="katb_use_gravatar_wrap">
					...
					<a href="https://en.gravatar.com/" title="Gravatar Site" target="_blank" >
						<img class="gravatar_logo" src="<?php echo plugins_url();// WPCS: XSS ok. ?>/testimonial-basics/includes/Gravatar80x16.jpg" alt="Gravatar Website" title="Gravatar Website" />
					</a>
				</span>
				<?php
			}
